...
(select sum(rbr_h_amnt_ok) from tblrbr where rbr_comref  = com_ref and rbr_date between '{startDate}' and '{endDate}') ) * 100 ,0),2) as 'UnpRatio',
...
and com_acc_cancel in (0,1) ) t

order by t.UnpRatio desc