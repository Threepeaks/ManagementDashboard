select 
    ...
    group by dbt_comref) unpaids    on unpaids.cref = com_ref

    where com_acc_cancel in (0,1) 

order by com_acc_cancel desc, round(ifnull(((unpaids.sumValue /collections.sumValue ) * 100),0),2) desc
